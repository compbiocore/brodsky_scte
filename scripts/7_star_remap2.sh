...
#SBATCH -n 16
#SBATCH -N 1
#SBATCH -e /gpfs/data/cbc/abrodsky/brodsky_scte/logs/scte_star_remap-%A-%a-%J.err
#SBATCH -o /gpfs/data/cbc/abrodsky/brodsky_scte/logs/scte_star_remap-%A-%a-%J.out
...
singularity exec /gpfs/data/cbc/abrodsky/brodsky_scte/metadata/brodsky_scte_20220128.sif STAR --runMode alignReads --outFilterMatchNminOverLread 0 --outFilterScoreMinOverLread 0 --readFilesCommand zcat --runThreadN 16 --soloStrand Unstranded --outFilterMatchNmin 0 --soloUMIdedup Exact NoDedup --winAnchorMultimapNmax 100 --outSAMmultNmax 1 --outSAMtype BAM Unsorted --genomeDir /gpfs/data/shared/databases/refchef_refs/grch38_release98/STAR_2_7_10a --outFileNamePrefix ${align_dir}/${sample_name}_permissive_ --readFilesIn ${fastq_dir}/${sample_name}_1.fastq.gz ${fastq_dir}/${sample_name}_2.fastq.gz
